 ...
 */
static int _nn_cmp_shift(nn_src_t in1, nn_src_t in2, u8 shift, int *cmp)
{
	int ret, mask, tmp;
	u8 i;
 ...
 */
static int _nn_cnd_sub_shift(int cnd, nn_t out, nn_src_t in,
 ...
 * in: it does not verify that.
 */
static int _nn_submul_word_shift(nn_t out, nn_src_t in, word_t w, u8 shift,
				word_t *borrow)
{
 ...
 */
static int _wcmp_22(word_t a[2], word_t b[2])
{
	int mask, ret = 0;
	ret += (a[1] > b[1]);
	ret -= (a[1] < b[1]);
 ...
 * Checks left to the caller.
 */
static word_t _wadd_22(word_t a[2], word_t b[2])
{
	word_t carry;
	a[0]  = (word_t)(a[0] + b[0]);
 ...
 * Checks left to the caller.
 */
static word_t _wsub_22(word_t a[2], word_t b[2])
{
	word_t borrow, tmp;
	tmp    = (word_t)(a[0] - b[0]);
 ...
 */
static int _word_divrem(word_t *q, word_t *r, word_t ah, word_t al, word_t b)
{
