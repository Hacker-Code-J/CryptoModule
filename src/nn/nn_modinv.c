 ...
 *
 */
static int _nn_modinv_odd(nn_t out, nn_src_t x, nn_src_t m)
{
 ...
 */
static inline int _nn_sub_mod_2exp(nn_t A, nn_src_t B)
{
	u8 Awlen = A->wlen;
	int ret;
 ...
 * hereafter.
 */
static int _nn_modinv_fermat_common(nn_t out, nn_src_t x, nn_src_t p, nn_t p_minus_two, int *lesstwo)
{
	int ret, cmp, isodd;
	nn two;
	two.magic = WORD(0);
